...
use super::dto::{
    AppliedMove, CellMoveRequest, GameMode, GameStateResponse, HvBStarter, Winner, status_hvb,
};
...
pub async fn create_game(
    ...
    // Validamos que exista, pero NO ejecutamos ya la jugada del bot.
...
pub async fn post_human_move(
    ...
    if session.mode != GameMode::Hvb {
        return Err(ApiErrorResponse::conflict("Game is not HvB", "invalid_game_mode"));
    }

    if session.hvb_winner.is_some() {
        return Err(ApiErrorResponse::conflict("Game already finished", "game_finished"));
    }
    ...
    if !next_is_human {
        return Err(ApiErrorResponse::conflict("It is not human turn", "not_human_turn"));
    }
    ...
    Ok(Json(serde_json::json!({
        ...
        "yen": crate::YEN::from(&session.game),
        "human_move": {
            "cell_id": human_applied.cell_id,
            "coords": human_applied.coords
        },
        "status": status_hvb(
            session.hvb_next_is_human.unwrap_or(true),
            session.hvb_winner,
...
pub async fn post_bot_move(
    ...
    if next_is_human {
        return Err(ApiErrorResponse::conflict("It is not bot turn", "not_bot_turn"));
    }
    ...
    Ok(Json(serde_json::json!({
        ...
        "yen": crate::YEN::from(&session.game),
        "bot_move": {
            "cell_id": bot_applied.cell_id,
            "coords": bot_applied.coords
        },
        "status": status_hvb(
            session.hvb_next_is_human.unwrap_or(true),
            session.hvb_winner,
