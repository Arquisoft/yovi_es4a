...
async function createVerifiedUser(username, email) {
  await request(app).post('/createuser').send({ username, password: '1234', email });
...
describe('POST /createuser', () => {
  ...
  it('returns an error if the user already exists', async () => {
    await request(app).post('/createuser').send({ username: 'Pablo2', password: '1234', email: 'pablo2@test.com' })
    const res = await request(app)
      .post('/createuser')
...
describe('POST /login', () => {
  beforeAll(async () => { await createVerifiedUser('LoginUser', 'login@test.com'); })

  it('returns a welcome message for valid credentials', async () => {
    const res = await request(app).post('/login').send({ username: 'LoginUser', password: '1234' }).set('Accept', 'application/json')
    expect(res.status).toBe(200)
    expect(res.body.message).toMatch(/Bienvenido LoginUser/i)
  })

  it('returns an error for incorrect password', async () => {
    const res = await request(app).post('/login').send({ username: 'LoginUser', password: 'wrongpassword' }).set('Accept', 'application/json')
    expect(res.status).toBe(401)
    expect(res.body.error).toMatch(/Contraseña incorrecta/i)
  })

  it('returns an error if the user does not exist', async () => {
    const res = await request(app).post('/login').send({ username: 'NoExiste', password: '1234' }).set('Accept', 'application/json')
    expect(res.status).toBe(401)
...
describe('PATCH /users/:username/stats', () => {
  beforeAll(async () => { await createVerifiedUser('StatsUser', 'stats@test.com'); })
  ...
  it('increments gamesLost and does NOT accumulate totalMoves when won=false', async () => {
    const res = await request(app)
      .patch('/users/StatsUser/stats')
    ...
    expect(res.body.stats.totalMoves).toBe(10) // unchanged — losses don't count
    expect(res.body.stats.winRate).toBe(50)
  })

  it('returns 404 for non-existent user', async () => {
    const res = await request(app).patch('/users/NoExiste/stats').send({ won: true, totalMoves: 5 })
    expect(res.status).toBe(404)
  })

  it('returns 400 if won is not a boolean', async () => {
    const res = await request(app).patch('/users/StatsUser/stats').send({ won: 'yes', totalMoves: 5 })
    expect(res.status).toBe(400)
  })

  it('returns 400 if totalMoves is negative', async () => {
    const res = await request(app).patch('/users/StatsUser/stats').send({ won: true, totalMoves: -1 })
    expect(res.status).toBe(400)
  })
})
...
describe('GET /ranking', () => {
  ...
  it('ranking entries include totalMoves (only from wins)', async () => {
    const res = await request(app).get('/ranking')
    const ranka = res.body.ranking.find(u => u.username === 'RankA')
    expect(ranka.totalMoves).toBe(24) // 10+8+6 from 3 wins
  })
