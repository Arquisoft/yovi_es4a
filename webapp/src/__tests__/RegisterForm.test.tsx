...
describe('RegisterForm Component', () => {
  ...
  it('debe renderizar todos los campos correctamente con sus labels e IDs', () => {
    ...
    expect(screen.getByLabelText('Contraseña', { selector: 'input#register_form_password' })).toBeInTheDocument();
  ...
  it('debe mostrar un mensaje de error si las contraseñas no coinciden al enviar', async () => {
    ...
    await user.type(screen.getByLabelText('Contraseña', { selector: 'input#register_form_password' }), 'Pass123!');
  ...
  it('debe mostrar error si el nivel de seguridad de la contraseña es demasiado bajo', async () => {
    ...
    await user.type(screen.getByLabelText('Contraseña', { selector: 'input#register_form_password' }), 'weak');
  ...
  it('debe permitir la selección de un avatar diferente', async () => {
    ...
    const avatar1 = screen.getByRole('button', { name: 'Seleccionar avatar Avatar 1' });
    const avatar2 = screen.getByRole('button', { name: 'Seleccionar avatar Avatar 2' });
  ...
  it('debe procesar el registro con éxito cuando los datos son válidos', async () => {
    ...
    await user.type(screen.getByLabelText(/Nombre de Usuario/i), 'user');
    await user.type(screen.getByLabelText(/Correo Electrónico/i), 'a@a.com');
    await user.type(screen.getByLabelText('Contraseña', { selector: 'input#register_form_password' }), 'Segura123!');
