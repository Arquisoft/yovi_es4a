...
describe("GameHvB", () => {
  ...
  it("normaliza starter=bot y respeta bot/size de la query", () => {
    mockSearchParams = new URLSearchParams(
      "size=9&bot=mcts_bot&hvbstarter=BoT",
    );
  ...
  it("hace fallback a size=7 y starter=human si la query es inválida", () => {
    mockSearchParams = new URLSearchParams(
      "size=1&bot=smart_bot&hvbstarter=alien",
    );
  ...
  it("genera textos de resultado correctos", () => {
    ...
    expect(props.resultConfig.getResultText("human")).toBe(
      "Has ganado la partida.",
    );
    expect(props.resultConfig.getResultText("bot")).toBe(
